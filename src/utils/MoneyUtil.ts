export class MoneyUtil {
  ...
  private readonly li: number;

  private constructor(cents: number) {
    this.cents = Math.round(cents);
    this.li = Math.round(cents * 10);
  }
  ...
  add(other: MoneyUtil | number): MoneyUtil {
    const otherCent =
      typeof other === 'number' ? Math.round(other * 1000) : other.cents;
    return new MoneyUtil(this.cents + otherCent);
  }

  static add(a: string, b: string): string {
    const aCent = Math.round(Number(a || 0) * 1000);
    const bCent = Math.round(Number(b || 0) * 1000);
    return ((aCent + bCent) / 1000).toFixed(3);
  }
